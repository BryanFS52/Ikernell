...
public class ReporteController {
    ...
    public ReporteController(InterrupcionRepository interrupcionRepository,
                             ...
                             PersonaProyectoService personaProyectoService,
    ...
    @GetMapping("/proyectos-y-trabajadores/csv")
    public ResponseEntity<String> generarReporteProyectosYTrabajadores() {
        // Obtener todas las asignaciones de persona-proyecto
        List<PersonaProyecto> asignaciones = personaProyectoService.listar();

        StringBuilder csv = new StringBuilder();
        csv.append("Nombre del Proyecto;Desarrollador(es)\n");

        // Iterar sobre cada asignación (persona-proyecto)
        for (PersonaProyecto asignacion : asignaciones) {
            String nombreProyecto = asignacion.getProyecto().getNombre();
            StringBuilder desarrolladores = new StringBuilder();

            // Obtener los desarrolladores asignados a cada proyecto
            desarrolladores.append(asignacion.getPersona().getNombre())
                    .append(" ")
                    .append(asignacion.getPersona().getApellido());

            // Agregar la fila al CSV
            csv.append(nombreProyecto).append(";")
                    .append(desarrolladores.toString()).append("\n");
        }

        // Devolver el archivo CSV como respuesta
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=proyectos_y_trabajadores.csv")
                .header(HttpHeaders.CONTENT_TYPE, "text/csv")
                .body(csv.toString());
    }
}
